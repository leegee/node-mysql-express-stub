...
function testGet (uri, callback){
	...
	.on('response', function (res) {
		...
		res.on('end', function(){
			...
			callback( JSON.parse(rawBody), res );
		});
	})
	.end();
};
...
describe('app', function(){
	...
	it('should be defined with an infline function', function (done) {
		model.should.be.type('object');
		done();
	});
});
...
describe('URIs', function(){
	...
	it('should error for a non-existant table', function (done) {
		testGet('http://localhost:3000/Notable'+(new Date().getTime()), function (body,res) {
			res.statusCode.should.be.equal(200);
	...
	it('should return wrapped results for an existant table, '+TEST_DATABASE+'.'+TEST_TABLE,
		function (done) {
			testGet('http://localhost:3000/'+TEST_TABLE, function (body) {
				body.should.not.have.property('error');
				body.should.have.property('results');
				body.results.length.ok;
				done();
	...
	it('should select by column', function (done) {
		var columnName = 'text';
		testGet('http://localhost:3000/'+TEST_TABLE+'/'+columnName, function(body){
			body.should.not.have.property('error');
			body.should.have.property('results');
			body.results.length.ok;
	...
	it('should select by column and column value', function (done) {
		var columnName = 'id';
		var columnValue = 1;
		testGet('http://localhost:3000/'+TEST_TABLE+'/'+columnName+'/'+columnValue, function(body){
			body.should.not.have.property('error');
			body.should.have.property('results');
			body.results.length.ok;
			body.results[0].should.be.an.instanceOf( Object );
	...
	it('should select by two column and column value', function (done) {
		testGet('http://localhost:3000/'+TEST_TABLE+'/id/1/id/1', function(body){
			body.should.not.have.property('error');
			body.should.have.property('results');
			body.results.length.ok;
			body.results[0].should.be.an.instanceOf( Object );
